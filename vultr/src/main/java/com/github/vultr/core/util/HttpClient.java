...
public class HttpClient {
    ...
    public byte[] get(final String url) throws IOException {
        return ReadDocuments(url, "GET", null).getData();
    }
    ...
    public byte[] post(final String url, final byte[] bin) throws IOException {
        return ReadDocuments(url, "POST", bin).getData();
    }
     ...
     */
    @SneakyThrows
    public ResultBean ReadDocuments(String url) {
        return ReadDocuments(url, "GET", null);
    }
     ...
     */
    @SneakyThrows
    public ResultBean ReadDocuments(String url, String method, byte[] postData) {
        return ReadDocuments(new URL(url), method, postData, null, null);
    }
     ...
     */
    @SneakyThrows
    public ResultBean ReadDocuments(String url, String method, byte[] postData, Map<String, String> requestHead) {
        return ReadDocuments(new URL(url), method, postData, requestHead, null);
    }


    public ResultBean ReadDocuments(final URL url, final String method, final byte[] postData,
                                    Map<String, String> requestHead, Boolean onlyHead) throws IOException {
        HttpURLConnection httpURLConnection = connect(url, method, requestHead);
     ...
     * @throws IOException
     */
    protected HttpURLConnection connect(final URL url, String method, Map<String, String> requestHead)
            throws IOException {
        ...
        // 设置访问模式
        httpURLConnection.setRequestMethod(!StringUtils.hasText(method) ? "GET" : method.trim().toUpperCase());
